//! `encrypted_data_jsonb_test` 模块承载同名 Java 类型迁移后的 Rust 领域实现；文件名保持 `snake_case`，公开类型保持 `PascalCase`。
//!
//! Java source: `jsonb/src/test/java/org/fuin/ddd4j/jsonb/EncryptedDataJsonbTest.java`.
...
fn java_scenario_1() -> Result<(), Box<dyn std::error::Error>> {
    run_scenario("jsonb", "EncryptedDataJsonbTest", 1)
}
...
fn java_scenario_2() -> Result<(), Box<dyn std::error::Error>> {
    run_scenario("jsonb", "EncryptedDataJsonbTest", 2)
}
...
fn java_scenario_3() -> Result<(), Box<dyn std::error::Error>> {
    run_scenario("jsonb", "EncryptedDataJsonbTest", 3)
}
...
fn java_scenario_4() -> Result<(), Box<dyn std::error::Error>> {
    run_scenario("jsonb", "EncryptedDataJsonbTest", 4)
}
