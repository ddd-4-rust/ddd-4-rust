//! `b_id` 模块承载同名 Java 类型迁移后的 Rust 领域实现；文件名保持 `snake_case`，公开类型保持 `PascalCase`。
//!
//! Java source: `jackson/src/test/java/org/fuin/ddd4j/jacksontest/BId.java`.

/// Executable compatibility fixture retaining the Java type identity.
#[derive(Debug, Clone, PartialEq, Eq)]
/// `BId` 表示与同名 Java 类型对应的 Rust 领域对象。
