...
//!
//! Java source: `jackson/src/test/java/org/fuin/ddd4j/jacksontest/PersonName.java`.

/// Serializer-specific alias of the shared executable test model.
///
/// `PersonName` 是迁移兼容层公开的稳定类型别名。
/// 该别名不复制底层数据，用于保持 Java 来源类型与 Rust 公共路径的一一对应关系。
pub type PersonName = ddd_4_rust_test_model::PersonName;
