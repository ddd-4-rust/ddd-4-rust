...
//!
use crate::{ModelError, VendorId, VendorRef, event_metadata::EventMetadata};
...
/// Domain event emitted when a vendor is created.
///
/// `VendorCreatedEvent` 表示与同名 Java 类型对应的 Rust 领域对象。
...
impl VendorCreatedEvent {
    ...
    pub fn new(vendor_ref: VendorRef) -> Result<Self, ModelError> {
        let id = Arc::new(*vendor_ref.id()) as Arc<dyn EntityId>;
        Ok(Self {
            metadata: EventMetadata::new("VendorCreatedEvent")?,
...
impl std::fmt::Display for VendorCreatedEvent {
    /// 按稳定且可读的格式输出当前值，不改变对象内部状态。
    /// 该方法不引入未声明的全局副作用，调用方应按签名处理返回结果。
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "Created vendor '{}'", self.vendor_ref)
    }
}
impl Event for VendorCreatedEvent {
...
impl DomainEvent<VendorId> for VendorCreatedEvent {
...
impl DomainEvent<dyn EntityId> for VendorCreatedEvent {
    /// 执行 `entity_id_path` 对应的领域行为，参数和返回值遵循当前类型公开契约。
    /// 该方法不引入未声明的全局副作用，调用方应按签名处理返回结果。
    fn entity_id_path(&self) -> &EntityIdPath {
        &self.path
